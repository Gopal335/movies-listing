...
export default async function ActorPage({ params }: { params: Params }) {
  ...
  const filtered = all.filter(m => m.cast.some(c => c.toLowerCase() === actor.toLowerCase()));

  if (!filtered.length) return notFound();

  return (
    <section>
      <h1 className="text-3xl font-bold mb-8">Movies starring {actor}</h1>
